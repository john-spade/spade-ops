...
export default (router) => {
    ...
    router.get('/dashboard/employee-stats', async (ctx) => {
        ...
        if (!employee) return ctx.error('Employee profile not found', 404);

        // Get next shift
        const [nextShift] = await db.raw(`
            SELECT s.*, sites.name as site_name, sites.location
            FROM shifts s
            JOIN sites ON s.site_id = sites.id
            WHERE s.employee_id = ? AND s.start_time > NOW()
            ORDER BY s.start_time ASC
            LIMIT 1
        `, [employee.id]);

        // Get unread messages
        const [messages] = await db.raw('SELECT COUNT(*) as count FROM messages WHERE recipient_id = ? AND is_read = FALSE', [ctx.state.user.id]);

        // Get unread notifications
        const [notifications] = await db.raw('SELECT COUNT(*) as count FROM notifications WHERE user_id = ? AND is_read = FALSE', [ctx.state.user.id]);

        ctx.success({
            nextShift: nextShift ? {
                site: nextShift.site_name,
                location: nextShift.location,
                start: nextShift.start_time,
                end: nextShift.end_time
            } : null,
    ...
    router.get('/payroll', async (ctx) => {
        ...
        // Filter for specific employee if logged in as employee
        if (ctx.state.user && ctx.state.user.role === 'employee') {
            const employee = await db.table('employees').where({ user_id: ctx.state.user.id }).first();
            if (employee) {
        ...
        query += ' GROUP BY employees.id, employees.first_name, employees.last_name, employees.position';

        const payroll = await db.raw(query, params) || [];
        ctx.success(payroll);
    });
